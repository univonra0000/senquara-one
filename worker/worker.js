...
async function adminUpsertUser(request,env){const s=await requireOwner(request,env);if(s.error)return s.error;const b=await body(request),id=str(b.id),name=str(b.name),email=str(b.email).toLowerCase();if(!name||!email)return json({ok:false,error:'Name and email are required.'},400);if(!ROLES.includes(str(b.role)))return json({ok:false,error:'Invalid role.'},400);let u=id?await env.DB.prepare('SELECT id FROM users WHERE id=?').bind(id).first():null;const now=new Date().toISOString();if(u){await env.DB.prepare('UPDATE users SET name=?,email=?,mobile=?,business_name=?,country=?,role=?,status=?,updated_at=? WHERE id=?').bind(name,email,str(b.mobile),str(b.businessName),str(b.country),str(b.role),['pending','active','blocked'].includes(str(b.status))?str(b.status):'pending',now,id).run()}else{const pw=str(b.password);if(pw.length<8)return json({ok:false,error:'New user password must contain at least 8 characters.'},400);const ph=await hashPassword(pw),nid=crypto.randomUUID();await env.DB.prepare('INSERT INTO users(id,email,name,mobile,business_name,country,password_hash,password_salt,role,status,email_verified,terms_version,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)').bind(nid,email,name,str(b.mobile),str(b.businessName),str(b.country),ph.hash,ph.salt,str(b.role),str(b.status)||'pending',1,'1.0',now,now).run();id=nid}await audit(env,s.user.id,'ADMIN_USER_UPSERT','users',id,{email});return json({ok:true,id});}
